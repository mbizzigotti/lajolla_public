...
Implement Yan et al.'s normal map filtering technique~\cite{Yan:2014:RGH} or Jakob's discrete stochastic microfacet models~\cite{https://rgl.epfl.ch/publications/Jakob2014Discrete} in lajolla. 
...
Focus on the rendering technology. Analyze the technology you used in the game and why you used them. Compared to alternatives.
...
\href{https://en.wikipedia.org/wiki/Crofton_formula}{Crofton formula} relates an integral over a curve to an integral over all lines that intersect with the curve. Thus we can use it for designing algorithms to sample points on the curve, which makes it very suitable for an edge sampling algorithm. We explored a (biased but consistent) version of this type of algorithm in our recent work~\cite{Belhe:2025:ASD}, but did not apply it to path tracing tasks. Can we use similar algorithms for edge sampling and how would it fair against alternatives like projective sampling?
...
Our recent work on Neural Geometry Fields~\cite{Edavamadathil:2024:NGF} proposed a data structure for storing geometric displacement using neural networks. However, we only explored rasterization for NGF rendering in the paper. How do we efficiently render a NGF with on-the-fly without tesselation~\cite{Thonat:2021:TDM}? Would the best rendering algorithm be different from the existing ones for displacement mapping?
