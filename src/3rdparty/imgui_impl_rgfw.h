...
void ImGui_ImplRgfw_KeyCallback(RGFW_window* window, u8 key, u8 modState, RGFW_bool repeat, RGFW_bool pressed)
{
    ImGui_ImplRgfw_Data* bd = ImGui_ImplRgfw_GetBackendData();
    if (bd->PrevUserCallbackKey != nullptr && ImGui_ImplRgfw_ShouldChainCallback(window))
        bd->PrevUserCallbackKey(window, key, modState, repeat, pressed);
...
void ImGui_ImplRgfw_NewFrame()
{
    ...
    i32 w, h;
    RGFW_window_getSizeInPixels(bd->Window, &w, &h);
